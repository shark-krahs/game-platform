...
const TetrisBoard: React.FC<GameBoardProps> = ({
  ...
  if (gameState.game_type !== "tetris") {
    return (
      <div style={{ padding: "20px", color: "#fff" }}>
        ...
        <p>
          This component is for Tetris games only. Game type:{" "}
          {gameState.game_type}
        </p>
      </div>
    );
  ...
  const canPlacePiece = useCallback(
    (piece: any, x: number, y: number, rotation: number): boolean => {
  ...
  useEffect(() => {
    if (!fallingPiece || gameState.status !== "playing" || !isPlayerTurn)
      return;
  ...
  const renderCell = (x: number, y: number) => {
    ...
    if (fallingPiece) {
      const tetromino =
        TETROMINOES[fallingPiece.type as keyof typeof TETROMINOES];
      if (tetromino) {
        ...
        if (
          pieceX >= 0 &&
          pieceX < (rotatedPiece[0]?.length || 0) &&
          pieceY >= 0 &&
          pieceY < rotatedPiece.length &&
          (rotatedPiece[pieceY]?.[pieceX] || 0) > 0
        ) {
          cellColor =
            PIECE_COLORS[fallingPiece.type as keyof typeof PIECE_COLORS] ||
            "#fff";
      ...
      {fallingPiece && isPlayerTurn && !readOnly && (
        <div style={{ marginTop: "20px" }}>
          ...
          >
            ...
            <button onClick={placePiece} style={downButtonStyle}>
              ↓
            </button>
            <button
        ...
        <div style={{ borderTop: "1px solid #555", paddingTop: "15px" }}>
          ...
          {gameState.board_state?.scores && (
            <div style={{ marginTop: "15px" }}>
              ...
              {gameState.board_state.scores.map(
                (score: number, index: number) => (
                  <div
                    key={index}
                    style={{ color: index === 0 ? "#ff0000" : "#0000ff" }}
                  >
