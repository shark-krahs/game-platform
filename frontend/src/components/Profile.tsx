...
const Profile: React.FC = () => {
  ...
  const [emailMessageType, setEmailMessageType] = useState<"success" | "info">(
    "info",
  );
  ...
  useEffect(() => {
    if (emailCooldown > 0) {
      localStorage.setItem(
        storageKey,
        String(Date.now() + emailCooldown * 1000),
      );
    } else {
      localStorage.removeItem(storageKey);
    }
      ...
      <Card>
        <Space direction="vertical" size="middle" style={{ width: "100%" }}>
          <Space direction="vertical">
            ...
            <Typography.Text>
              <UserOutlined /> <strong>{t("email")}:</strong>{" "}
              {user?.email || t("missingEmail" as any)}
            </Typography.Text>
            <Typography.Text>
      ...
      >
        ...
        >
          ...
          <Form.Item name="preferredColor" label={t("preferredColor")}>
            <Input type="color" style={{ width: 60, height: 40, padding: 4 }} />
          </Form.Item>
      ...
      <Card title={t("emailChange")}>
        <Space direction="vertical" style={{ width: "100%" }}>
          ...
          {emailChangeRequested && emailCooldown > 0 && (
            <Typography.Text type="secondary">
              {t("resendCooldown" as any, {
                time: formatSeconds(emailCooldown),
              })}
            </Typography.Text>
          )}
          ...
          <Button
            ...
            onClick={async () => {
              const confirmed = window.confirm(
                t("confirmEmailChangePrompt" as any),
              );
              if (!confirmed) return;
              try {
                const response = await requestEmailChange();
