...
const ConfirmEmailChange: React.FC = () => {
  ...
  const onFinish: FormProps<ConfirmEmailChangeValues>["onFinish"] = async (
    values,
  ) => {
    const token = searchParams.get("token");
    if (!token) {
    ...
    try {
      ...
      const messageText =
        (err as any)?.message || t("emailChangeConfirmationFailed" as any);
      ...
      <Space orientation="vertical" style={{ width: "100%" }}>
        ...
        {step === 1 && (
          <>
            ...
            {resendMessage && (
              <Alert type="info" message={resendMessage} showIcon />
            )}
