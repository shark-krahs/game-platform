...
import { App, Button, Col, Row, Slider, Spin } from "antd";
import {
  LeftOutlined,
  PauseCircleOutlined,
  PlayCircleOutlined,
...
const GameReplay: React.FC = () => {
  ...
  const getCurrentGameState = useCallback((): GameState => {
    ...
    if (savedGame.moves && savedGame.moves.length > 0) {
      ...
      if (targetMoveIndex >= 0 && targetMoveIndex < savedGame.moves.length) {
        ...
        if (
          move &&
          move.board_state_after !== undefined &&
          move.board_state_after !== null
        ) {
          try {
            // Use the board state after this move
            currentBoardState =
              typeof move.board_state_after === "string"
            ...
            if (move.time_remaining_after) {
              timeRemainingSnapshot = move.time_remaining_after as Record<
                number,
                number
              >;
            }
    ...
    if (
      savedGame.game_type === "pentago" &&
      currentBoardState &&
      currentBoardState.grid
    ) {
    ...
    return {
      ...
      winner:
        winner !== undefined ? savedGame.players[winner]?.name : undefined,
  ...
  const parseChatHistory = (
    history: SavedGameDetail["chat_history"],
  ): ChatMessage[] => {
    if (!history) {
      return [];
    }
  ...
  const replayPlayers = savedGame.players.map((player, index) => {
    const moveSnapshot =
      currentMoveIndex > 0
        ? savedGame.moves[currentMoveIndex - 1]?.time_remaining_after
        : null;
    const remaining =
      moveSnapshot && typeof moveSnapshot === "object"
        ? ((moveSnapshot as Record<number, number>)[index] ??
  ...
  return (
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "20px", textAlign: "center" }}>
        ...
        <p>
          {savedGame.game_type} -{" "}
          {savedGame.players.map((p) => p.name).join(" vs ")}
        </p>
      </div>
