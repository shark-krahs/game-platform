...
const ForgotPassword: React.FC = () => {
  const { requestPasswordReset, resendPasswordReset, loading, error } =
    useAuth();
  ...
  const onFinish: FormProps<ForgotPasswordValues>["onFinish"] = async (
    values,
  ) => {
    try {
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        {infoMessage && (
          <Alert
            ...
            message={infoMessage}
            description={
              maskedEmail ? t("emailSentTo", { email: maskedEmail }) : undefined
            }
            showIcon
          />
        )}
        {infoMessage && cooldown > 0 && (
          <Typography.Text type="secondary">
            <ClockCircleOutlined />{" "}
            {t("resendCooldown", { time: formatSeconds(cooldown) })}
          </Typography.Text>
        )}
        {infoMessage && (
