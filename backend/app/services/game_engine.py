...
async def broadcast_state(game_id: str, game_state: GameState):
    ...
    if game_state.game_type == "pentago":
        ...
        state["board"] = (
            game_state.board_state.get("grid", []) if game_state.board_state else []
        )
...
class GameEngine:
            ...
            if player1_id_str and player2_id_str:
                if is_bot_player(game_state.players[0]) or is_bot_player(
                    ...
                    try:
                        ...
                        logger.info(
                            f"Updated ratings for bot game {game_id}: winner={game_state.winner}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to update ratings for bot game {game_id}: {e}"
                        )
                else:
                    try:
                        ...
                        logger.info(
                            f"Updated ratings for game {game_id}: winner={game_state.winner}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to convert user IDs to UUID for ratings update: {e}"
                        )
        ...
        for player in game_state.players:
            if player.get("user_id") and not is_bot_player(player):
                try:
                    ...
                    logger.info(
                        f"Saving game {game_id} for user {player['name']} (ID: {user_id})"
                    )
                    ...
                    logger.error(
                        f"Failed to auto-save game {game_id} for user {player.get('name', 'unknown')}: {e}"
                    )
                    import traceback
    ...
    async def _cleanup_finished_game_after_delay(
        ...
        if game_id in self.finished_games:
            ...
            logger.info(
                f"Cleaned up finished game {game_id} after {delay_seconds} seconds"
            )
    ...
    async def create_game(
        ...
        logger.info(
            f"Created {game_type} game {game_id} with time control {time_control_str}"
        )
        return game_state
    ...
    async def process_move(
        ...
        if valid:
            ...
            if new_state.status == "first_move":
                ...
                logger.info(
                    f"Game {game_id} first move phase: move {new_state.first_move_count}/2 completed"
                )
            ...
            if (
                ...
                move = GameMove(
                    ...
                    move_data={
                        "type": "turn_completed"
                    },  # Generic turn completion marker
                ...
                logger.info(
                    f"Game {game_id} turn completed by player {old_current_player}, recorded in history"
                )
        ...
        while game_id in self.active_games:
            ...
            if game_state.status == "waiting" and len(game_state.players) == 2:
                if game_state.game_type == "tetris":
                    ...
                    logger.info(
                        f"Game {game_id} Tetris started directly in playing mode"
                    )
                else:
                    ...
                    logger.info(
                        f"Game {game_id} first move initialized for player {game_state.first_move_player}"
                    )
            ...
            if game_state.status == "first_move":
                ...
                if game_state.first_move_timer <= 0:
                    ...
                    logger.info(
                        f"Game {game_id} first move timeout, player {game_state.first_move_player} loses"
                    )
            ...
            # Handle playing mode games (Pentago timer)
                ...
                game_state.status in ["playing", "disconnect_wait"]
            ...
            # Handle Tetris falling piece
                ...
                if not game_state.board_state.get("falling_piece"):
                    ...
                    logger.info(
                        f"Game {game_id} started falling piece for player {game_state.current_player}"
                    )
                ...
                if game_state.board_state.get("falling_piece"):
                    ...
                    if not game_state.board_state.get(
                        ...
                        if not game_state.board_state.get("next_pieces"):
                            ...
                            logger.info(
                                f"Game {game_id} started new piece for player {next_player}"
                            )
                ...
                if game_state.time_remaining[game_state.current_player] <= 0:
                    ...
                    game_state.status = "finished"
                    logger.info(
                        f"Game {game_id} timeout, player {game_state.current_player} loses"
                    )
            ...
            if (
                ...
                if game_state.disconnect_timer <= 0:
                    ...
                    winner_index = (game_state.disconnected_player + 1) % len(
                        game_state.players
                    )
