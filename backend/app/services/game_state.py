...
async def create_matched_game(
    ...
    time_control_obj = TimeControl(
        type="move_time", initial_time=total_time, increment=increment
    )
    ...
    if bot_difficulty is not None:
        ...
        if is_bot_player(players[0]) or is_bot_player(players[1]):
            asyncio.create_task(
                schedule_bot_move(game_state, game_id, bot_difficulty, selected_engine)
            )
...
async def handle_player_join(
    game_id: str, user_id: str, username: str, websocket: Any
) -> bool:
    """
    ...
    if game_id.startswith("match_"):
        ...
        if user_id not in allowed_users:
            await websocket.send_text(
                json.dumps({"type": "error", "message": "Not allowed in this game"})
            )
            return False
    ...
    if (
        ...
        and game_state.disconnected_player == player_index
    ):
    ...
    if game_state and game_state.status in ["playing"]:
        ...
        if connected_count == 0:
            ...
            if disconnected_player_index is not None:
                ...
                logger.info(
                    f"Game {game_id} player {user_id} disconnected, starting 30s reconnection timer"
                )
                await broadcast_state(game_id, game_state)
