...
class PentagoBoard(AbstractGameBoard):
    ...
    def initialize_board(self) -> Dict[str, Any]:
        ...
        return {
            'grid': [[None for _ in range(self.BOARD_SIZE)] for _ in range(self.BOARD_SIZE)],
    ...
    def check_winner(self, board_state: Dict[str, Any]) -> Optional[int]:
        ...
        for start_row in range(self.BOARD_SIZE - 3):
            for start_col in range(self.BOARD_SIZE - 3):
                # Main diagonal
                diagonal = [grid[start_row + step][start_col + step] for step in range(4)]
                winner = check_line(diagonal)
                if winner is not None:
                    return winner

                # Anti-diagonal
                anti_diagonal = [grid[start_row + step][start_col + 3 - step] for step in range(4)]
    ...
    def _rotate_quadrant(self, grid: List[List[Optional[int]]], quadrant: int, direction: str) -> None:
        ...
        # Rotate the quadrant
        rotated = [[None for _ in range(self.QUADRANT_SIZE)] for _ in range(self.QUADRANT_SIZE)]
