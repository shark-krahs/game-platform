...
class PentagoGame(AbstractGameLogic):
    ...
    def initialize_game(
        ...
        return GameState(
            ...
            time_remaining={
                i: float(time_control.initial_time) for i in range(len(players))
            },
            winner=None,
            moves_history=[],
            chat_history=[],
    ...
    def process_move(
        ...
        # Apply move
        new_state.board_state = self.board.apply_move(
            new_state.board_state, move, player_id
        )
        ...
        # Record move
        game_move = GameMove(
            player_id=player_id, move_data=move, timestamp=datetime.now()
        )
        ...
        if new_state.status == "first_move":
            ...
            if moves_made == 1:
                ...
                new_state.first_move_player = (new_state.current_player + 1) % len(
                    new_state.players
                )
                ...
                logger.info(
                    f"Game {new_state.game_id} both players made first moves, starting normal play"
                )
        ...
        if winner is not None:
            ...
            if new_state.status != "finished":
                new_state.current_player = (new_state.current_player + 1) % len(
                    new_state.players
                )
